...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        def replace(tag_to_delete, tag_to_use_instead):
            self.stdout.write(u'Replacing {} with {}'.format(title_pk(tag_to_delete),
                                                             title_pk(tag_to_use_instead)))

            topics = Topic.objects.filter(tags__pk=tag_to_delete['pk'])
            for topic in topics:
                self.stdout.write(u'    Replaced on topic "{}"'.format(topic))
            ...
            for content in contents:
                self.stdout.write(u'    Replaced on content "{}"'.format(content))
                content.tags.add(tag_to_use_instead.pk)
                content.tags.remove(tag_to_delete['pk'])

            self.stdout.write(u'    Deleting {}'.format(title_pk(tag_to_delete)))
        ...
        for title, tag in tags_tmp_dict.iteritems():
            ...
            if tag_to_use_instead:
                ...
                stripped.save()
                self.stdout.write(u'Stripped {} to {}'.format(title_pk(tag), title_pk(stripped)))
