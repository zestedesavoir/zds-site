...
class CustomSearchForm(ModelSearchForm):
    ...
    def model_choices(self, using=DEFAULT_ALIAS):
        choices = [("%s.%s" % (m._meta.app_label, m._meta.module_name), self.get_model_name(m._meta.app_label, m._meta.module_name, True)) for m in connections[using].get_unified_index().get_indexed_models()]
