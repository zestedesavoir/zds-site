...
class ManagerTests(TestCase):
        ...
        def test_get_unread_post(self):
            ...
            topic.save()
            TopicRead(post=topic.last_post, user=self.staff.user, topic=topic)
            self.assertEqual(0, len(TopicRead.objects.list_read_topic_pk(self.staff.user)))
            self.assertEqual(1, len(TopicRead.objects.list_read_topic_pk(author.user)))
