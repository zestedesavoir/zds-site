...
from zds.member.views import MemberList, MemberDetail, ProfileUpdate, UpdateGitHubToken, remove_github_token, \
    update_avatar, UpdatePasswordMember, UpdateUsernameEmailMember, RegisterView, \
    SendValidationEmailView, modify_karma, modify_profile, ProfileModeration, member_from_ip, \
...
urlpatterns = [
    ...
    # modification
    url(r'^parametres/profil/$', ProfileUpdate.as_view(), name='update-member'),
    ...
    url(r'^parametres/profil/maj_avatar/(?P<image_pk>\d+)/$', update_avatar, name='update-avatar'),
    ...
    url(r'^profil/moderer/(?P<pk>\d+)/$', ProfileModeration.as_view(), name='member-moderation'),
