...
class ArticleTests(TestCase):
    ...
    def test_add_reaction(self):
        ...
        reaction1 = ReactionFactory(article=self.article, position=2, author=self.staff)
    ...
    def test_url_for_guest(self):
        ...
        result = self.client.get(
                        reverse('zds.article.views.view_online', args=[self.article.pk, self.article.slug]),
                        follow=True)
        self.assertEqual(result.status_code, 200)
                
        #guest can't read offline articles
        result = self.client.get(
                        reverse('zds.article.views.view', args=[self.article.pk, self.article.slug]),
                        follow=False)
        self.assertEqual(result.status_code, 302)
