...
def maj_repo_article(
    ...
    if action == 'del':
        ...
        if aut_email is None or aut_email.strip() == '':
            aut_email = u'inconnu@{}'.format(settings.ZDS_APP['site']['dns'])
    ...
    if request.user.has_perm('article.change_article'):
        ...
        if 'reject-article' in request.POST:
            ...
            if request.user == validation.validator:
                ...
                messages.error(request,
                               u'Vous devez avoir réservé cet article pour pouvoir le refuser.')
                return redirect(
                    article.get_absolute_url() +
                    '?version=' +
                    validation.version)
        ...
        # must update sha_public with the current sha of the validation.
            ...
            if request.user == validation.validator:
                ...
                messages.error(request,
                               u'Vous devez avoir réservé cet article pour pouvoir le publier.')
                return redirect(
                    article.get_absolute_url() +
                    '?version=' +
                    validation.version)
