...
class ContentTests(TestCase):
    def setUp(self):
        ...
        overrided_zds_app['content']['default_licence_pk'] = self.licence.pk
    ...
    def test_import_image_with_archive(self):
        ...
        prefix = overrided_zds_app['content']['import_image_prefix']
        ...
        for img in Image.objects.filter(gallery=new_article.gallery).all():
            self.assertTrue('![]({})'.format(overrided_zds_app['site']['secure_url'] + img.physical.url) in text)
    ...
    def test_validation_workflow(self):
        ...
        different_source = u'http://example.org'
        text_accept = u"C'est cool, merci !"
        text_reject = u'Je refuse ce contenu.'
    ...
    def test_warn_typo(self):
        ...
        typo_text = u"Il y a une faute, c'est triste !"
        ...
        result = self.client.post(
            reverse('validation:accept', kwargs={'pk': validation.pk}),
            {
                'text': u"ça m'a l'air intéressant, je valide",
                'is_major': True,
                'source': u''
            },
            follow=False)
    ...
    def test_publication_make_extra_contents(self):
        ...
        overrided_zds_app['content']['build_pdf_when_published'] = True  # obviously, PDF builds have to be enabled
