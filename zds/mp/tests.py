...
class MPTests(TestCase):
    ...
    def test_create_mp(self):
        ...
        result = self.client.get(reverse('zds.mp.views.topic', args=[ptopic.pk, ptopic.pk]),follow=True)
    ...
    def test_edit_mp_post(self):
        ...
        ppost1 = PrivatePostFactory(privatetopic=ptopic1, author=self.user1, position_in_topic = 1)
        ppost2 = PrivatePostFactory(privatetopic=ptopic1, author=self.user1, position_in_topic = 2)
        ppost3 = PrivatePostFactory(privatetopic=ptopic1, author=self.user1, position_in_topic = 3)
        ...
        #check values
        self.assertEqual(PrivatePost.objects.get(pk=ppost3.pk).text, u'C\'est tout simplement l\'histoire de la ville de Paris que je voudrais vous conter ')
