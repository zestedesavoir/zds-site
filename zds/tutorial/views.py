...
def reservation(request, validation_pk):
    ...
    if validation.validator:
        ...
        messages.info(
            request,
            u'Le tutoriel a bien été réservé par {0}.'.format(
...
def modify_tutorial(request):
    ...
    if request.user in tutorial.authors.all():
        if 'add_author' in request.POST:
            ...
            messages.success(
                request,
                u'L\'auteur {0} a bien été retiré du tutoriel.'.format(
                    author.username))
...
def modify_part(request):
    ...
    if 'move' in request.POST:
        ...
        old_slug = os.path.join(
            os.path.join(
                settings.REPO_PATH, str(
                    part.tutorial.pk) + '_' + part.tutorial.slug), part.slug)
...
def view_chapter(request, tutorial_pk, tutorial_slug, part_slug,
    ...
    prev_chapter = chapter_tab[
        final_position -
        1] if final_position > 0 else None
        ...
        # Edit extract.
            ...
            if form.is_valid():
                ...
                if extract.chapter.tutorial:
                    ...
                    chapter_part = os.path.join(
                        chapter_tutorial_path,
                ...
                # Get path for big-tuto
                    ...
                    chapter_part_path = os.path.join(
...
def maj_repo_part(
    ...
    if action == 'del':
        ...
        intro.close()
        index.add(
            [os.path.join(part.get_path(relative=True), 'introduction.md')])
...
def maj_repo_chapter(
    ...
    if(chapter.tutorial):
        ...
        repo = Repo(
            os.path.join(
                settings.REPO_PATH, str(
                    chapter.part.tutorial.pk) + '_' + chapter.part.tutorial.slug))
...
def maj_repo_extract(
    ...
    if(extract.chapter.tutorial):
        man_path = os.path.join(
            extract.chapter.tutorial.get_path(),
            'manifest.json')
        extract.chapter.tutorial.dump_json(path=man_path)
    else:
        man_path = os.path.join(
            extract.chapter.part.tutorial.get_path(),
            'manifest.json')
...
def download(request):
    ...
    response[
        'Content-Disposition'] = 'attachment; filename={0}.tar'.format(tutorial.slug)
...
def download_markdown(request):
    ...
    response[
        'Content-Disposition'] = 'attachment; filename={0}.md'.format(tutorial.slug)
...
def download_html(request):
    ...
    response = HttpResponse(
        ...
        mimetype='text/html')
    response[
        'Content-Disposition'] = 'attachment; filename={0}.html'.format(tutorial.slug)
...
def download_pdf(request):
    ...
    response[
        'Content-Disposition'] = 'attachment; filename={0}.pdf'.format(tutorial.slug)
...
def download_epub(request):
    ...
    response[
        'Content-Disposition'] = 'attachment; filename={0}.epub'.format(tutorial.slug)
    ...
    if request.method == 'POST':
        ...
        if 'preview' in request.POST:
            ...
            form.helper.form_action = reverse(
                'zds.tutorial.views.edit_note') + '?message=' + str(note_pk)
