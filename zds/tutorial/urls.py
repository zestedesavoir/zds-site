...
urlpatterns = [
    ...
    # Current URLs
    url(r'^recherche/(?P<pk_user>\d+)/$', 'zds.tutorial.views.find_tuto'),

    url(r'^off/(?P<tutorial_pk>\d+)/(?P<tutorial_slug>.+)/(?P<part_pk>\d+)/(?P<part_slug>.+)/(?P<chapter_pk>\d+)/(?P<ch'
        r'apter_slug>.+)/$', 'zds.tutorial.views.view_chapter',  name="view-chapter-url"),

    url(r'^off/(?P<tutorial_pk>\d+)/(?P<tutorial_slug>.+)/(?P<part_pk>\d+)/(?P<part_slug>.+)/$',
        'zds.tutorial.views.view_part', name="view-part-url"),

    url(r'^off/(?P<tutorial_pk>\d+)/(?P<tutorial_slug>.+)/$', 'zds.tutorial.views.view_tutorial'),

    # Beta URLs
    url(r'^beta/(?P<tutorial_pk>\d+)/(?P<tutorial_slug>.+)/(?P<part_pk>\d+)/(?P<part_slug>.+)/(?P<chapter_pk>\d+)/(?P<c'
        r'hapter_slug>.+)/$', 'zds.tutorial.views.view_chapter_beta', name="view-chapter-url-beta"),

    url(r'^beta/(?P<tutorial_pk>\d+)/(?P<tutorial_slug>.+)/(?P<part_pk>\d+)/(?P<part_slug>.+)/$',
        'zds.tutorial.views.view_part_beta', name="view-part-url-beta"),

    url(r'^beta/(?P<tutorial_pk>\d+)/(?P<tutorial_slug>.+)/$', 'zds.tutorial.views.view_tutorial_beta'),

    # View online
    url(r'^(?P<tutorial_pk>\d+)/(?P<tutorial_slug>.+)/(?P<part_pk>\d+)/(?P<part_slug>.+)/(?P<chapter_pk>\d+)/(?P<chapte'
        r'r_slug>.+)/$', 'zds.tutorial.views.view_chapter_online', name="view-chapter-url-online"),

    url(r'^(?P<tutorial_pk>\d+)/(?P<tutorial_slug>.+)/(?P<part_pk>\d+)/(?P<part_slug>.+)/$',
        'zds.tutorial.views.view_part_online', name="view-part-url-online"),

    url(r'^(?P<tutorial_pk>\d+)/(?P<tutorial_slug>.+)/$', 'zds.tutorial.views.view_tutorial_online'),
    ...
    url(r'^historique/(?P<tutorial_pk>\d+)/(?P<tutorial_slug>.+)/$', 'zds.tutorial.views.history'),
    url(r'^comparaison/(?P<tutorial_pk>\d+)/(?P<tutorial_slug>.+)/$', 'zds.tutorial.views.diff'),

    # user actions
    url(r'^suppression/(?P<tutorial_pk>\d+)/$', 'zds.tutorial.views.delete_tutorial'),
    url(r'^validation/tutoriel/$', 'zds.tutorial.views.ask_validation'),

    # Validation
    url(r'^validation/$', 'zds.tutorial.views.list_validation'),
    url(r'^validation/reserver/(?P<validation_pk>\d+)/$', 'zds.tutorial.views.reservation'),
    url(r'^validation/reject/$', 'zds.tutorial.views.reject_tutorial'),
    url(r'^validation/valid/$', 'zds.tutorial.views.valid_tutorial'),
    url(r'^validation/invalid/(?P<tutorial_pk>\d+)/$', 'zds.tutorial.views.invalid_tutorial'),
    url(r'^validation/historique/(?P<tutorial_pk>\d+)/$', 'zds.tutorial.views.history_validation'),
    url(r'^activation_js/$', 'zds.tutorial.views.activ_js'),
    # Reactions
    ...
    url(r'^message/typo/(?P<obj_type>.+)/(?P<obj_pk>\d+)/$', 'zds.tutorial.views.warn_typo'),

    # Moderation
    url(r'^resolution_alerte/$', 'zds.tutorial.views.solve_alert'),

    # Help
    url(r'^aides/$', 'zds.tutorial.views.help_tutorial'),
]
