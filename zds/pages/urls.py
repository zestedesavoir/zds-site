...
urlpatterns = [
    ...
    re_path(r'^contenu-original/(?P<pk>\d+)/$',
            EditDetail.as_view(), name='edit-detail'),
    re_path(r'^restaurer-edition/(?P<edit_pk>\d+)/$',
            restore_edit, name='restore-edit'),
    re_path(r'^supprimer-contenu-edition/(?P<edit_pk>\d+)/$',
            delete_edit_content, name='delete-edit-content'),
