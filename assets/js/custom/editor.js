(function(){
	/* zForm Script, by Thunderseb. Je hais jQuery */
	...
	var zForm = {
		...
		init: function() {
			...
			var wrapper = document.body.appendChild(document.createElement("div")), raw = '';
				wrapper.id = "zform-modal-wrapper";			
			
			raw += '<div><header id="zform-modal-header"></header>';
			raw += '<section class="zform-modal" id="zform-modal-link">' +
				'<div>' + 
			...
			raw += '<section class="zform-modal" id="zform-modal-image">' +
				'<div>' +
			...
			raw += '<section class="zform-modal" id="zform-modal-abbr">' +
				'<div>' +
			...
			raw += '<section class="zform-modal" id="zform-modal-footnote">' +
				'<div>' +
			...
			raw += '<footer><a id="zform-modal-validate">Valider</a> <a id="zform-modal-cancel">Annuler</a></footer></div>';
	
			wrapper.innerHTML = raw;
			
			this.addEvent(document.getElementById("zform-modal-validate"), "click", (function(_this) { return function() {
				_this.validatePopup();
			}; }) (this));
			
			this.addEvent(document.getElementById("zform-modal-cancel"), "click", (function(_this) { return function() {
				_this.closePopup();
			}; }) (this));
		},
		...
		setup: function(textareaId) {
			...
			if (!this.isExecuted) {
				this.addEvent(document, "click", function(event) {
					if (~event.target.className.indexOf("zform-button")) {
						if (!(~event.target.className.indexOf("zform-subbutton"))) {
			...
			for (var g=0, cg=groups.length; g<cg; g++) {
				...
				for (var b=0, cb=buttons.length; b<cb; b++) {
					...
					//elemButton.innerHTML = buttons[b] || currentButton.title.charAt(0);
		...
		tagType: function(selection, type, options, isFromPopup) {
			...
			if (!selection.startTag)	selection.startTag	= "";
			if (!selection.endTag)		selection.endTag	= "";
