...
(function(document, $) {
  ...
  function sendMoveAction(evt) {
    ...
    $('*[data-children-type]').each(function() {
      if ($(this).children(':not(.simple-create-button, .simple-create-part)')[0]) { return }

      if (!$(this).parent().is('.article-content > .article-part')) { return }
      ...
      onMove: function(evt) {
        ...
        if (!$to.is($from)) {
          ...
          if (childrenType === 'extract') {
            const pathChap = ($obj) => $obj.is('.article-containers > .article-part')
            const pathPartChap = ($obj) => $obj.is('.article-content > .article-part')
            // 1) pathChap: from: [chapter] > extract
            // 2) pathPartChap: from: part > [chapter] > extract

            // Element is dragged into the list from another list (with different level)
            if (pathChap($from.parent()) && pathPartChap($to)) {
              console.log('B2')
              $item.find('> h4 > a').unwrap().wrap('<h3></h3>')
            } else if (pathPartChap($from.parent()) && pathChap($to)) {
