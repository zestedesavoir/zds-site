...
function getAnswerText(liWrapper) {
  const label = liWrapper.querySelector('.answer-label')
  const MathAnnotation = label.querySelector('annotation')
  let answer
  if (MathAnnotation) {
  ...
  form.addEventListener('submit', e => {
    ...
    if (QuizzAnswered(form)) {
      ...
      Object.keys(answers).forEach(name => {
        ...
        const title = getQuestionText(element.querySelector('.custom-block-heading'))
