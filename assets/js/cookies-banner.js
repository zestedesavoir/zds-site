...
(function(document, undefined) {
    ...
    var gaTrackingId = $("meta[name=gaTrackingId]").attr("content").split(":");

    function checkHasConsent(){
        if(document.cookie.indexOf("hasconsent=true") > -1){
            if (gaTrackingId && gaTrackingId.length === 2)
                $("#gtm").after(
                    "<script>" +
                        "dataLayer = [{'gaTrackingId': '" + gaTrackingId[0] + "'}];" +
                        "(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':" +
                        "new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0]," +
                        "j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src=" +
                        "'//www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);" +
                        "})(window,document,'script','dataLayer','" + gaTrackingId[1] + "');" +
                    "</script>"
                );
