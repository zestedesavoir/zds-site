...
RELATIVE_PIC_NAME = "easy-thumbnails-test_source.jpg"
PIC_NAME = os.path.join(settings.MEDIA_ROOT, RELATIVE_PIC_NAME)
THUMB_NAME = os.path.join(settings.MEDIA_ROOT, "easy-thumbnails-test_%02d.jpg")
PIC_SIZE = (800, 600)
...
class BaseTest(unittest.TestCase):
    ...
    def verify_thumbnail(self, expected_size, thumbnail=None,
        ...
        if isinstance(thumb_name, six.string_types):
            # Verify that the thumbnail file exists
            self.assertTrue(
                os.path.isfile(thumb_name), 'Thumbnail file not found')
