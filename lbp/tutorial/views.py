...
from lbp.gallery.models import Gallery, UserGallery, Image
from lbp.utils import render_template, slugify
from lbp.utils.models import Version, DateManager, Category, Licence
...
def list(request):
    ...
    try:
        tutorials = Tutorial.objects.all() \
            .filter(version__public_version_number__lte=0) \
...
def edit_tutorial(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            #add next version
            tutorial.draft_version_number=tutorial.version.draft_version_number+1
...
def modify_tutorial(request):
    ...
    if request.user in tutorial.authors.all():
        if 'add_author' in request.POST:
            redirect_url = reverse('lbp.tutorial.views.edit_tutorial') + \
